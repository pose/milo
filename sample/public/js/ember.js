...
*/
    function watchedEvents(obj) {
      var listeners = obj[META_KEY].listeners,
        ret = [];
...
*/
  ...
  */
      ...
      controllerAt: function (idx, object, controllerClass) {
        ...
        if (!subController) {
          subController = container.lookup("controller:" + controllerClass, {
            singleton: false
          });
      ...
      string: function () {
        if (this._hasElement && this._element) {
          ...
          var thisElement = this.element(),
            outerHTML = thisElement.outerHTML;
...
*/
  ...
  */
      nearestOfType: function (klass) {
        var view = get(this, 'parentView'),
          isOfType = klass instanceof Ember.Mixin ? function (view) {
    ...
    Ember.merge(states.hasElement, {
      ...
      ensureChildrenAreInDOM: function (view) {
        var childViews = view._childViews,
          i, len, childView, previous, buffer, viewCollection = new ViewCollection();
    ...
    Ember.Handlebars.registerBoundHelper = function (name, fn) {
      ...
      function helper() {
        ...
        for (hashOption in hash) {
          if (!hash.hasOwnProperty(hashOption)) {
            continue;
          }
...
*/
    ...
    /** @scope Ember.Select.prototype */
    {
      ...
      defaultTemplate: Ember.Handlebars.template(function anonymous(Handlebars, depth0, helpers, partials, data) {
        ...
        data = data || {};
        var buffer = '',
          stack1, hashTypes, escapeExpression = this.escapeExpression,
          self = this;
...
*/
  ...
  (function () {
    define("router", ["route-recognizer"],
    ...
    */
      function setupContexts(router, handlerInfos) {
        var partition = partitionHandlers(router.currentHandlerInfos || [], handlerInfos);
...
*/
    ...
    Ember.controllerFor = function (container, controllerName, context, lookupOptions) {
      return container.lookup('controller:' + controllerName, lookupOptions) || Ember.generateController(container, controllerName, context);
    };
    /*
  ...
  */
      transitionTo: function (name, context) {
        ...
        if (this._checkingRedirect) {
          this._redirected[this._redirectDepth] = true;
        }
...
*/
    ...
    Ember.onLoad('Ember.Handlebars', function (Handlebars) {
      ...
      function resolvedPaths(options) {
        ...
        return resolveParams(options.context, options.params, {
          types: types,
          data: data
        });
      }
      ...
      function args(linkView, router, route) {
        var passedRouteName = route || linkView.namedRoute,
          routeName;
      ...
      var LinkView = Ember.LinkView = Ember.View.extend({
        ...
        active: Ember.computed(function () {
          var router = this.get('router'),
            ...
            isActive = router.isActive.apply(router, [this.currentWhen].concat(params)) || router.isActive.apply(router, [currentWithIndex].concat(params));
...
*/
    ...
    Ember.ControllerMixin.reopen({
      ...
      controllers: Ember.computed(function () {
        return ControllersProxy.create({
          controller: this
        });
      })
    });
        ...
        while (contexts.length > 0) {
          if (stateIdx >= 0) {
            ...
            if (this.enterStates.length) {
              ...
              if (!state) {
                throw "Cannot match all contexts to states";
              }
            } else {
  ...
  */
      removeUnchangedContexts: function (manager) {
        ...
        while (this.enterStates.length > 0) {
          if (this.enterStates[0] !== this.exitStates[0]) {
            break;
          }

          if (this.enterStates.length === this.contexts.length) {
            if (manager.getStateMeta(this.enterStates[0], 'context') !== this.contexts[0]) {
              break;
            }
            this.contexts.shift();
          }
