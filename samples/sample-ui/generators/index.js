...
var scaffolt = module.exports = function(type, name, options, callback) {
  ...
  var path = parentPath ? parentPath.replace('app/', '') : inflection.pluralize(type);
  ...
  var templateData = {name: name, pluralName: pluralName, path: path};

  checkIfExists(generatorsPath, function(exists) {
    exports.generateFiles(revert, generatorsPath, type, templateData, undefined, function(error) {
      if (error != null) {
        logger.error(error);
